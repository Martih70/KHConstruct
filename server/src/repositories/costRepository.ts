...
export const categoriesRepository = {
  getAll: (): CostCategory[] => {
    try {
      ...
      `);
      return stmt.all() as CostCategory[];
    } catch (error) {
  ...
  getById: (id: number): CostCategory | null => {
    try {
      ...
      const result = stmt.get(id) as CostCategory | undefined;
      return result || null;
    } catch (error) {
  ...
  getByCode: (code: string): CostCategory | null => {
    try {
      ...
      const result = stmt.get(code) as CostCategory | undefined;
      return result || null;
    } catch (error) {
  ...
  create: (data: {
    ...
    try {
      ...
      const created = categoriesRepository.getById(result.lastInsertRowid as number);
...
export const subElementsRepository = {
  getAll: (): CostSubElement[] => {
    try {
      ...
      `);
      return stmt.all() as CostSubElement[];
    } catch (error) {
  ...
  getByCategoryId: (categoryId: number): CostSubElement[] => {
    try {
      ...
      `);
      return stmt.all(categoryId) as CostSubElement[];
    } catch (error) {
  ...
  getById: (id: number): CostSubElement | null => {
    try {
      ...
      const result = stmt.get(id) as CostSubElement | undefined;
      return result || null;
    } catch (error) {
  ...
  getByCode: (code: string): CostSubElement | null => {
    try {
      ...
      const result = stmt.get(code) as CostSubElement | undefined;
      return result || null;
    } catch (error) {
  ...
  create: (data: {
    ...
    try {
      ...
      const created = subElementsRepository.getById(result.lastInsertRowid as number);
      if (!created) throw new Error('Failed to create sub-element');
      return created;
    } catch (error) {
  ...
  update: (
    ...
    try {
      ...
      if (updates.length === 0) {
        ...
        if (!existing) throw new Error('Sub-element not found');
        return existing;
      }
      ...
      if (!updated) throw new Error('Failed to update sub-element');
      return updated;
    } catch (error) {
...
export const unitsRepository = {
  ...
  getById: (id: number): Unit | null => {
    try {
      ...
      const result = stmt.get(id) as Unit | undefined;
      return result || null;
    } catch (error) {
  ...
  getByCode: (code: string): Unit | null => {
    try {
      ...
      const result = stmt.get(code) as Unit | undefined;
      return result || null;
    } catch (error) {
...
export const costItemsRepository = {
  getAll: (databaseType?: string): CostItem[] => {
    try {
      ...
      return params.length > 0 ? (stmt.all(...params) as CostItem[]) : (stmt.all() as CostItem[]);
    } catch (error) {
  ...
  getById: (id: number): CostItem | null => {
    try {
      ...
      const result = stmt.get(id) as CostItem | undefined;
      return result || null;
    } catch (error) {
  ...
  getBySubElementId: (subElementId: number): CostItem[] => {
    try {
      ...
      const stmt = db.prepare(`
        ...
        ORDER BY code ASC
      `);
      return stmt.all(subElementId) as CostItem[];
    } catch (error) {
  ...
  getByCategoryId: (categoryId: number): CostItem[] => {
    try {
      ...
      `);
      return stmt.all(categoryId) as CostItem[];
    } catch (error) {
  ...
  search: (filters: {
    ...
    try {
      ...
      return stmt.all(...values) as CostItem[];
    } catch (error) {
  ...
  create: (data: {
    ...
    try {
      ...
      const result = stmt.run(
        ...
        data.unit_id,
        data.material_cost,
        data.management_cost || 0,
        data.contractor_cost || 0,
        data.is_contractor_required ? 1 : 0,
        data.volunteer_hours_estimated || null,
        data.waste_factor || 1.05,
        data.currency || 'GBP',
      ...
      const created = costItemsRepository.getById(result.lastInsertRowid as number);
      if (!created) throw new Error('Failed to create cost item');
      return created;
    } catch (error) {
  ...
  update: (
    ...
    try {
      ...
      if (!updated) throw new Error('Failed to update cost item');
      return updated;
    } catch (error) {
