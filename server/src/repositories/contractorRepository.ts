...
export const contractorsRepository = {
  getAll: (userId: number, filters?: ContractorFilters): BuildingContractor[] => {
    try {
      ...
      return stmt.all(...params) as BuildingContractor[];
    } catch (error) {
  ...
  getById: (id: number, userId: number): BuildingContractor | null => {
    try {
      ...
      const contractor = stmt.get(id, userId) as BuildingContractor | undefined;
  ...
  getByName: (name: string, userId: number): BuildingContractor | null => {
    try {
      ...
      const contractor = stmt.get(name, userId) as BuildingContractor | undefined;
      return contractor || null;
    } catch (error) {
  ...
  create: (userId: number, data: CreateContractorRequest): BuildingContractor => {
    try {
      ...
      const contractor = contractorsRepository.getById(result.lastInsertRowid as number, userId);
      if (!contractor) {
    ...
    try {
      ...
      if (data.name !== undefined) {
        ...
        const other = db
          ...
          .get(data.name, userId, id) as BuildingContractor | undefined;
        if (other) {
  ...
  delete: (id: number, userId: number): boolean => {
    try {
      ...
      const projects = db.prepare('SELECT COUNT(*) as count FROM projects WHERE contractor_id = ?').get(id) as {
        count: number;
      };
      if (projects.count > 0) {
