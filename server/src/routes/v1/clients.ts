 ...
 * Create a new client
 */
router.post('/', verifyAuth, validate(createClientSchema), (req: Request, res: Response) => {
  try {
    const user = (req as any).user;
 ...
 * Update a client
 */
router.put('/:id', verifyAuth, validate(updateClientSchema), (req: Request, res: Response) => {
  try {
    const user = (req as any).user;
    const { id } = req.params;
    const clientId = parseInt(id, 10);
