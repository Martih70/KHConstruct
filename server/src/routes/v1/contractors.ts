 ...
 */
router.post('/', verifyAuth, validate(createContractorSchema), (req: Request, res: Response) => {
  try {
    const user = (req as any).user;
 ...
 * Update a contractor
 */
router.put('/:id', verifyAuth, validate(updateContractorSchema), (req: Request, res: Response) => {
  try {
    const user = (req as any).user;
    const { id } = req.params;
    const contractorId = parseInt(id, 10);
