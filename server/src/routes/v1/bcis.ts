 ...
 */
router.get('/elements', verifyAuth, (_req: Request, res: Response) => {
  try {
 ...
 */
router.get('/elements/:id', verifyAuth, (req: Request, res: Response) => {
  try {
    const db = getDatabase();
    const { id } = req.params;
 ...
 */
router.get('/sub-elements/:id', verifyAuth, (req: Request, res: Response) => {
  try {
    const db = getDatabase();
    const { id } = req.params;
 ...
 */
router.get('/search', verifyAuth, (req: Request, res: Response) => {
  try {
    const db = getDatabase();
 ...
 */
router.get('/structure', verifyAuth, (_req: Request, res: Response) => {
  try {
    const db = getDatabase();
    ...
    const elementsStmt = db.prepare(`
      ...
      ORDER BY sort_order ASC
    `);
    const elements = elementsStmt.all() as any[];
    ...
    res.json({
      ...
      subElementCount: structure.reduce((sum, e) => sum + (e.subElements as any[]).length, 0),
    });
  } catch (error: any) {
 ...
 */
router.get('/items/:elementCode', verifyAuth, (req: Request, res: Response) => {
  try {
    const db = getDatabase();
    ...
    const elementStmt = db.prepare(
      "SELECT id FROM cost_categories WHERE code = ? AND code LIKE 'BCIS-%'"
    );
    const element = elementStmt.get(elementCode) as { id: number } | undefined;
 ...
 */
router.get('/item/:id', verifyAuth, (req: Request, res: Response) => {
  try {
    const db = getDatabase();
    const { id } = req.params;
    ...
    const totalCostPerUnit = (item as any).material_cost + (item as any).management_cost + (item as any).contractor_cost;
