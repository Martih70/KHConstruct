 ...
 */
router.get('/', verifyAuth, (_req: Request, res: Response) => {
  try {
...
router.post(
  ...
  validate(createSubElementSchema),
  (req: Request, res: Response) => {
    try {
      ...
      logger.info(
        `Sub-element created: ${subElement.code} in category ${category.code} by user ${req.user?.username}`
      );
      res.status(201).json({
        success: true,
        data: subElement,
...
router.put(
  ...
  verifyAuth,
  authorize('admin', 'estimator'),
  validate(updateSubElementSchema),
  (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const subElementId = parseInt(id, 10);
