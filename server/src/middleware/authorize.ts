  ...
  return (req: Request, res: Response, next: NextFunction): void => {
    try {
      ...
      if (!allowedRoles.includes(req.user.role)) {
        logger.warn(
          `Forbidden: User ${req.user.username} (${req.user.role}) tried to access ${req.method} ${req.path}`
        );
        res.status(403).json({
          success: false,
 ...
 */
export function isOwner(resourceUserId: number) {
  return (req: Request, res: Response, next: NextFunction): void => {
    try {
      if (!req.user) {
        res.status(401).json({
      ...
      if (req.user.role !== 'admin' && req.user.userId !== resourceUserId) {
        logger.warn(
          `User ${req.user.username} attempted to access resource owned by user ${resourceUserId}`
        );
        res.status(403).json({
          success: false,
