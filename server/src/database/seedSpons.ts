 ...
 */
export function seedSponsDatabase(db: Database): void {
  try {
    // Check if Spons costs already exist
    const checkStmt = db.prepare(
      "SELECT COUNT(*) as count FROM cost_items WHERE database_type = 'spons'"
    );
    ...
    if (!categoryId) {
      ...
      const catResult = catInsertStmt.run(
        'SPONS-001',
        'Spons Detailed Cost Base',
        'Spon\'s Architects\' and Builders\' Price Book 2026 - Detailed UK Construction Costs',
        101
      );
    ...
    for (let i = 0; i < subElements.length; i++) {
      ...
      const existing = subCheckStmt.get(sub.code) as { id: number } | undefined;
      if (existing) {
