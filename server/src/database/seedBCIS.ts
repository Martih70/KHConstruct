...
/**
 * BCIS (Building Cost Information Service) Database Seeding
 * Implements the standard UK construction cost framework
 * Based on 2024-2025 UK market rates
 */

interface BCISElement {
  code: string;
  name: string;
  description: string;
}

interface BCISSubElement {
  elementCode: string;
  code: string;
  name: string;
  description: string;
...
interface BCISItem {
  ...
  region?: string;
  bcisReference?: string;
}

const BCIS_ELEMENTS: BCISElement[] = [
  {
    code: 'BCIS-A',
    name: 'Substructure',
...
const BCIS_SUB_ELEMENTS: BCISSubElement[] = [
  // Substructure (A)
  {
    elementCode: 'BCIS-A',
    code: 'BCIS-A1',
...
export function seedBCISDatabase(db: Database): void {
  try {
    // Check if BCIS cost items already seeded (more reliable than checking just categories)
    const checkStmt = db.prepare(
      "SELECT COUNT(*) as count FROM cost_items WHERE database_type = 'bcis'"
    );
    ...
    const elementInsertStmt = db.prepare(
    ...
    for (const subElement of BCIS_SUB_ELEMENTS) {
      ...
      if (categoryId) {
        ...
        subElementMap.set(subElement.code, result.lastInsertRowid as number);
        subElementCount++;
      }
    }
